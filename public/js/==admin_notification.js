        ...
        onSnapshot(q, (snapshot) => {
            ...
            snapshot.docChanges().forEach((change) => {
                ...
                if (change.type === 'removed') {
                    // Remove notification if document is deleted
                    if (existingItem) {
                        existingItem.remove();
                    }
    ...
    try {
        ...
        if (newStatus === 'Approved') {
            try {
                ...
                showDocumentPreview(documentData, collectionName);
            } catch (previewError) {
                console.error('Error in showDocumentPreview:', previewError);
                throw previewError;
            }
        } else {
        ...
        // Update the requests display if the function exists
...
function showDetailsPopup(data, collectionName) {
    ...
    const requestType = data.requestType || collectionName.replace('brgy_', '').toUpperCase(); // Fallback to a derived value if requestType is not available
